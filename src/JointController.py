...
class JointCommands_msg_handler(object):
    ...
    JointNames = ["compass_biped::inner_ankle","compass_biped::left_hip","compass_biped::left_ankle","compass_biped::right_hip","compass_biped::right_ankle"]


    def __init__(self):

        self.Joint_dict = {JointCommands_msg_handler.JointNames[i]:i for i in xrange(len(JointCommands_msg_handler.JointNames))}
        ...
        self._compub = rospy.Publisher('/compass_biped/robot_command', RobotCommand)
    ...
    def set_default_gains_from_yaml(self,yaml_path):
        ...
        self._default_gains = self._default_gains['robot_controller']['gains']

    def set_default_gains_from_param(self,param_name = '/robot_controller'):
    ...
    def reset_gains(self,joints = 'ALL'):
        ...
        for i in lst:
          ...
          self._command.kp_position[i]  = self._default_gains[name[15::]]['p']
          self._command.kd_position[i]  = self._default_gains[name[15::]]['d']
          self._command.ki_position[i]  = self._default_gains[name[15::]]['i']
          self._command.i_effort_max[i] = self._default_gains[name[15::]]['i_clamp']
    ...
    def set_pos(self,joint, pos):
        if type(joint) == int:
            ...
            joint_num = self.Joint_dict['compass_biped::'+joint]
        else:
            raise TypeError
            return
